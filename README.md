...
- **3LO is agent-side, not Gateway-mediated — by choice.** Verified @2026-08-19 the Gateway *does* perform per-user 3LO for a `CustomOauth2` provider like Lark, once its execution role carries `bedrock-agentcore:GetResourceOauth2Token` / `GetWorkloadAccessToken*` **and** `secretsmanager:GetSecretValue` on `bedrock-agentcore-identity!default/oauth2/*`. Without those it returns one opaque `An internal error occurred`, which is what earlier notes here mistook for a missing feature. This variant keeps the agent-driven path because that is what is built and measured; see `docs/agentcore-behavior.md`.
