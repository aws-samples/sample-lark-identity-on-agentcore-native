...
Built-in vendors with a dedicated config sub-key (endpoints baked in, you supply only client id/secret): Google, Github, Slack, Salesforce, Microsoft, Atlassian, Linkedin. The rest use `includedOauth2ProviderConfig` (you also supply authorization/token/issuer endpoints). Reported by community write-ups to emit a Gateway elicitation: Github, Linkedin — second-hand, and the `-32042` code those reports use does not appear in AWS docs (which list `-32600`/`-32601`/`-32021` plus `URLElicitationRequiredError`). We have never verified Gateway-mediated per-user delivery first-hand for any vendor. The agent-side flow below works for any built-in vendor and is the more portable pattern.
...
**Verified @2026-08-18:** an earlier version called Gateway-mediated per-user elicitation for `CustomOauth2` "the one real remaining AWS gap". It is not. 3LO on a `CustomOauth2` Gateway target completes and the target reaches `READY`; the real limit is that a token never reaches an **AgentCore Runtime** target's container (no `authorization` header, no token header). `agentcore-samples#1424`'s opaque error reproduces from a gateway role missing `bedrock-agentcore:InvokeAgentRuntime`. See `docs/agentcore-behavior.md` for the four experiments. Either way this document's agent-driven flow remains the portable choice, and it is the only one that works when the target is a Runtime. Extra shim requirement for a non-standard token endpoint like Lark's: the shim's `/token` must forward PKCE `code_verifier` to the upstream.
