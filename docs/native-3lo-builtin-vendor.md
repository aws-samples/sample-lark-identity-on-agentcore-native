...
Built-in vendors with a dedicated config sub-key (endpoints baked in, you supply only client id/secret): Google, Github, Slack, Salesforce, Microsoft, Atlassian, Linkedin. The rest use `includedOauth2ProviderConfig` (you also supply authorization/token/issuer endpoints). Gateway-mediated per-user delivery is verified first-hand @2026-08-19 for **`CustomOauth2`** (the least-supported category), so built-in vendors have no reason to fare worse. The `-32042` code community write-ups cite is real, even though AWS's docs list only `-32600`/`-32601`/`-32021` plus `URLElicitationRequiredError`. The agent-side flow below works for any built-in vendor and is the more portable pattern.
...
**Verified @2026-08-19:** an earlier version called Gateway-mediated per-user elicitation for `CustomOauth2` "the one real remaining AWS gap". It is not — per-user 3LO on a `CustomOauth2` **OpenAPI** target works end to end (elicitation → consent → the Gateway injects that user's token), given the gateway role has the Identity permissions. The real limit is the target type: a token cannot reach an **AgentCore Runtime** target's container at all, because the Runtime's `/invocations` endpoint owns the `Authorization` header for its own transport auth (verified as a single-variable A/B, permissions ruled out). `agentcore-samples#1424`'s opaque error reproduces from a missing gateway-role permission. See `docs/agentcore-behavior.md` for the experiments. Either way this document's agent-driven flow remains the portable choice, and it is the only one that works when the downstream is Runtime-hosted. Extra shim requirement for a non-standard token endpoint like Lark's: the shim's `/token` must forward PKCE `code_verifier` to the upstream.
