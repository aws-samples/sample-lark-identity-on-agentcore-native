...
- **Within a session, the microVM and its MCP server process are reused — not cold-started per call.** Measured: 5 consecutive calls on one session hit the same instance id with a per-process counter incrementing 1→5. The first call for a target pays the cold start + MCP `initialize`; later same-session calls skip both. The two lifecycle limits govern different things, and only one is resettable:
  ...
  - `maxLifetime` (default 28800 s / 8 h) — **wall-clock age of the microVM from creation**, not the session. It never resets, so a continuously busy turn is still cut off at 8 h; `HealthyBusy` does not help here. This is the hard ceiling on any background task. Whether a microVM can be created before it is assigned to a session — which would leave less than the full 8 h — is not established; leave margin rather than designing against the ceiling.

Both are configurable 60–28800 s via `lifecycleConfiguration` and left at defaults on both Runtimes here. On either limit the `runtimeSessionId` survives — the next call provisions a fresh microVM (sanitized memory, cold start), and the session stays valid until the Runtime ARN is deleted. Since memory bills continuously across a session including idle, a shorter idle timeout is the direct lever on idle cost.
