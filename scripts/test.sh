#!/usr/bin/env bash
# Run all unit tests. Each suite runs in its own process because the Lambda and
# agent dirs each define an `index.py`/`identity.py` — running them in a single
# pytest session would cross-import the wrong module.
set -euo pipefail
cd "$(cd "$(dirname "$0")/.." && pwd)"
